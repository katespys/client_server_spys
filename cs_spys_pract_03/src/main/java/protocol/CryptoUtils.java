...
public class CryptoUtils {
    ...
    private static byte[] getKey() {
        ...
        if (envKey == null || envKey.trim().isEmpty()) {
            return "TestSecretKey123".getBytes(StandardCharsets.UTF_8);
        }
        byte[] keyBytes = envKey.getBytes(StandardCharsets.UTF_8);
        if (keyBytes.length != 16 && keyBytes.length != 24 && keyBytes.length != 32) {
            throw new IllegalArgumentException("CRITICAL ERROR: ENCRYPT_KEY length must be exactly 16, 24, or 32 bytes long for AES.");
        }
        return keyBytes;
    }
